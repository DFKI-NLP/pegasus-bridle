...
if [ -f "$ENV_FILE" ]; then
    echo ".pegasus-bridle.env file found in current directory. load environment variables from $ENV_FILE"
else
    ENV_FILE="$SCRIPT_DIR"/.env
    echo "no .pegasus-bridle.env file found in current directory. use environment variables from $ENV_FILE"
fi
